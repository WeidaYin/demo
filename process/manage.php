...
class Manage extends \Fend\Fend
{
    ...
    public function monitorStart()
    {
        ...
        while (1) {
            ...
            if ($ret) {
                ...
                $pid    = $ret['pid'];
                \Fend\Log::write("monitor exit, pid=" . $pid . ",restart it!", $this->config['logName']);
                $this->pid = $this->newMonitor();
            }
            sleep(1);
        }
    ...
    private function newMonitor()
    {
        ...
        \Fend\Log::write('start monitor process', $this->config['logName']);
        $pid = $process->start();
        if (!$pid) {
            \Fend\Log::write('start  monitor  process  faild,please  check  memory or swoole\'s log', $this->config['logName']);
            return false;
        }
        return $pid;
    }
    ...
    {
        \Fend\Log::write('kill all process', $this->config['logName']);
        foreach ($this->table as $index => $item) {
            \swoole_process::kill($item['pid']);
        }
        \Fend\Log::write('kill monitor', $this->config['logName']);
        exit(0);
    }
    ...
    {
        \Fend\Log::write('kill monitor process', $this->config['logName']);
    ...
    public function childDie()
    {
        while ($ret = \swoole_process::wait(false)) {
            ...
            \Fend\Log::write('a work process has been gone,wait monitor restart', $this->config['logName']);
        }
    }
    ...
    private function killMe()
    {
        ...
        $this->table->del($key);
        $this->exiting();
        \Fend\Log::write('work process has been killed,pid is '.$pid, $this->config['logName']);
        exit;
    }
    ...
    private function dumpTable()
    {
        ...
        \Fend\Log::write(json_encode($statics));
    }
}
