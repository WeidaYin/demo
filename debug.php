...
class Debug extends \Fend\Fend
{
    public static $in=null;//保存当前对象
    ...
    public function dump()
    {
        ...
        $debug_error = \Fend\Di::factory()->get('debug_error');
        if (!empty($debug_error)) {
            $item .= $debug_error;
        }

        if (!empty($this->cfg['dbdebug'])) {
            $k = 0;
            $item .= "<table class='FD_table'>";
            $item .= "<tr class='bg'><th width='65'>OutType:</th><td>DateBase Info</td></tr>";
            settype($this->cfg['dbdebug'], 'array');
            foreach ($this->cfg['dbdebug'] as $v) {
                $item .= "<tr><th>#" . ++$k . ":</th><td>";
