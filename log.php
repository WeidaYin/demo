...
class Log
{
    ...
    public static function write($msg, $file = 'fend.log', $debug = 0)
    {
        ...
        // 写入日志
        if (!file_exists(SYS_ROOTDIR. "logs/fend/")) {
            mkdir(SYS_ROOTDIR . "logs/fend/", 0777, 1);
        }
        file_put_contents(SYS_ROOTDIR . "logs/fend/" . $file, date("Y-m-d H:i:s > ") . "{$msg}\n", FILE_APPEND);
    }
    ...
    public static function rawwrite($msg, $file = 'raw.log', $debug = 0)
    {
        ...
        // 写入日志
        if (!file_exists(SYS_ROOTDIR. "logs/")) {
            mkdir(SYS_ROOTDIR . "logs/", 0777, 1);
        }
        // 写入日志
        file_put_contents(SYS_ROOTDIR . "logs/" . $file, $msg . "\n", FILE_APPEND);
    }
    ...
    private static function recordLog($logname, $tag, $file, $line, $msg, $extra = array())
    {
        ...
        $log = array(
            "x_name" => $logname,
            "x_trace_id" => \Fend\EagleEye::getTraceId(),
            "x_rpc_id" => \Fend\EagleEye::getCurrentRpcId(),
            "x_version" => \Fend\EagleEye::getVersion(),
            "x_timestamp" => time(),
            "x_module" => $tag,
            "x_uid" => \Fend\EagleEye::getRequestLogInfo("uid"),
            "x_pid" => getmypid(),
